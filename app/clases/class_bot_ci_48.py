...
class botCi48(Thread):
    def __init__(self, bymaCI, byma48h, minimum_arbitrage_rate, maximum_arbitrage_rate, f, id_bot, cuenta):#f=instancia de fix, id_bot= id del bot que se está ejecutando
        ...
        self.log = logging.getLogger(f"botCi48_{id_bot}")
            ...
            if verificarOrdenCreada["status"]==True:#si tengo orden creada
                ...
                if verificarOperar["puedoOperar"]==True:
                    ...
                    if orden['price'] != limit_price_CI or orden['leavesQty'] != volume_limit_CI:#verifico si el precio o size son diferentes del q tengo actualmente
                        ...
                        if self.botData["ordenOperada"]==0:#verifico si hay una orden operada en proceso
                            if sideBook=="BI":
                            ...
                            self.log.error(f"hay una orden operada en proceso")
                    else: 
    ...
    async def verificar_cada_x_segundos(self, segundos):
        try: 
            ...
            if tiempoLimitStatus>=segundos:
                ...
                self.cola.agregar_tarea({"type": 0})
    ...
    async def procesar_tarea(self, tarea):
        try: 
            ...
            if typeTarea==0: 
                ...
                await self.verificar_orden_operada(tarea["data"])
            else:
                pass
            await self.cola.marcar_completada(tarea)
        except Exception as e: 
    ...
    def run(self):
        ...
        loop.run_until_complete(self.ejecutar())#ejecuto la funcion q quiero en este caso la de ejecutar_bot
        loop.close()
    ...
    def insertar_tarea_no_async(self, type, data ):
        try: 
            ...
            self.cola.agregar_tarea({"type": type, "data": data, "id": self.botData["contadorTareas"]})
        except Exception as e: 
    ...
    async def operar_orden_contraria(self, orden, symbolCheck, sideCheck, id_order, sideOrder):
        ...
        try: 
            ...
            #   self.botData["ordenesBot"].append({"idOperada":id_order, "clOrdId": clOrdId, "size": size })
                ...
                if ordenNew["data"]["ordStatus"]=="FILLED":
                    self.log.info(f"se completo bien la contraria ahora cancelo lo q haya en esa punta si es q hay")
                    cancelar_orden = await self.clientR.cancelar_orden_haberla(symbolCheck, sideOrder)
                    self.log.info(f"cancelar_orden: {cancelar_orden}")
            else:
                size = orden["lastQty"]
