...
class botCi48(Thread):
    ...
    async def verificar_48h(self, side):
        ...
        if verificarOrdenCreada["status"]==True:#si tengo orden creada
            ...
            if verificarOperar["status"]==True:
                ...
                if round(orden['price'],1) != round(limit_price_CI,1) or orden['leavesQty'] != volume_limit_CI:#verifico si el precio o size son diferentes del q tengo actualmente
                    ...
                    if self.botData["ordenOperada"] == False:#verifico si hay una orden operada en proceso
                        modificarOrden = await self.clientR.modificar_orden_rest(orden['orderID'], orden['clOrdId'],sideOrder, 2, self.botData["byma48h"],
                ...
                if side=="Buy":
                    ...
                    cancelarOrden = await self.clientR.cancelar_orden_rest(orden['orderID'], orden['clOrdId'], 1, 2, self.botData["byma48h"])
                    ...
                    cancelarOrden = await self.clientR.cancelar_orden_rest(orden['orderID'], orden['clOrdId'], 2, 2, self.botData["byma48h"])
            ...
            if sideBook=="BI":
                ...
                if posicionBymaCI<=0:
                    self.log.info("no hay nada en CI BI o debo")#x ende no puedo crear una orden de compra 48 xq no tengo nada en ci posicion
            ...
            if verificarOperar["status"]==True:#si puedo operar
                ...
                if self.botData["ordenOperada"] == False:#verifico si hay una orden operada en proceso
                    if posicionBymaCI>=volume_limit_CI:
                        ...
                        ordenNueva = await self.clientR.nueva_orden_rest(self.botData["byma48h"],sideOrder, volume_limit_CI, limit_price_CI, 2)#creo la orden
        ...
        if verificarOrdenCreada["status"]==True:#si tengo orden creada
            ...
            if verificarOperar["status"]==True:
                ...
                if round(orden['price'],1) != round(limit_price_CI,1) or orden['leavesQty'] != volume_limit_CI:#verifico si el precio o size son diferentes del q tengo actualmente
                    self.log.info("si el precio o size son diferentes del q tengo actualmente entonces modifico la orden")
                    if self.botData["ordenOperada"] == False:#verifico si hay una orden operada en proceso
                        modificarOrden = await self.clientR.modificar_orden_rest(orden['orderID'], orden['clOrdId'],sideOrder, 2, self.botData["bymaCI"],
                                                volume_limit_CI, limit_price_CI) #modifico la orden
                        self.log.info(f"orden modificada {modificarOrden}")
                    else:
                        self.log.info(f"hay una orden operada en proceso")
                ...
                if side=="Buy":
                    ...
                    cancelarOrden = await self.clientR.cancelar_orden_rest(orden['orderID'], orden['clOrdId'], 1, 2, self.botData["bymaCI"])
                    self.log.info(f"cancelar orden en ci buy: {cancelarOrden}")
                else:#es sell
                    self.log.info("cancelar orden en ci sell")
                    self.log.info(f"cancelar orden en ci sell: {orden['orderID']}")
                    cancelarOrden = await self.clientR.cancelar_orden_rest(orden['orderID'], orden['clOrdId'], 2, 2, self.botData["bymaCI"])
            ...
            if verificarOperar["status"]==True:#si puedo operar
                ...
                if self.botData["ordenOperada"] == False:#verifico si hay una orden operada en proceso
                    ...
                    ordenNueva = await self.clientR.nueva_orden_rest(self.botData["bymaCI"], sideOrder, volume_limit_CI,
    ...
    async def  verificar_orden_operada(self, details, type=0):
        ...
        await self.guardar_mitad_rueda(details, details["symbol"], details["side"])
        self.log.info("ahora operar la contraria ")
        await self.operar_orden(details, details["orderId"])
        self.botData["ordenOperada"] = False
                    
    async def guardar_mitad_rueda(self, details, symbol, side, descontar=0):
        ...
        if symbol==self.botData["byma48h"] and side=="Buy":
            ruedaType = "ruedaB"
            ruedaContraria = "ruedaA"
        elif symbol==self.botData["bymaCI"] and side=="Sell":
            ruedaType = "ruedaB"
            ruedaContraria = "ruedaA"
    ...
    async def operar_orden_contraria(self, orden, symbolCheck, sideCheck, id_order, sideOrder):
        ...
        if verifyF["status"]==True:
         ...
         #   self.botData["ordenesBot"].append({"idOperada":id_order, "clOrdId": clOrdId, "size": size })
            ordenNew = await self.clientR.nueva_orden_rest(symbolCheck, sideOrder, size, priceOrder, 2)
            #ordenNew = self.clientR.fix.newOrderSingle(clOrdId,symbolCheck, sideOrder, size, priceOrder, 2 )
            if ordenNew["status"]==1:
                if ordenNew["order"]["status"]=="FILLED":
                    self.log.info("es operada, osea q es una contraria correctamente ejecutada")
                    await self.guardar_mitad_rueda(ordenNew["order"], ordenNew["order"]["instrumentId"]["symbol"], ordenNew["order"]["side"], 1 )
                elif ordenNew["order"]["status"]=="NEW":
                    self.log.info("se quedo pegada la orden, vamos a guardarla para despues despegarla")
                    await self.guardar_orden_pegada(ordenNew["order"], id_order, ordenNew["order"]["clOrdId"] )
          ...
          #  ordenNew = self.clientR.fix.newOrderSingle(clOrdId,symbolCheck, sideOrder, volume_limit, limit_price, 2 )
            ordenNew = await self.clientR.nueva_orden_rest(symbolCheck, sideOrder, volume_limit, limit_price, 2)
            #ordenNew = self.clientR.fix.newOrderSingle(clOrdId,symbolCheck, sideOrder, size, priceOrder, 2 )
            if ordenNew["status"]==1:
                if ordenNew["order"]["status"]=="FILLED":
                    self.log.info("es operada, osea q es una contraria correctamente ejecutada")
