...
class botLento(Thread):
    ...
    async def  calcular_limit_futuro1_ask(self, verificarFuturo1):
        ...
        futuro1Ask = 0
        if verificarFuturo1["status"]==True:
        ...
        if futuro1Ask > 0 and futuro1Ask > precioMaxGanAskFuturo1 and (futuro1Ask-precioMaxGanAskFuturo1)>0.11:
            ...
            if verificarFuturo1x["status"]==True:
            ...
            limitTrueMax = float(self.clientR.fix.securitysList[self.botData["futuro1"]]["highLimitPrice"])
            limitTrueMin = float(self.clientR.fix.securitysList[self.botData["futuro1"]]["lowLimitPrice"])
    ...
    async def  calcular_limit_futuro1_bid(self, verificarFuturo1):
        ...
        if futuro1Bid>0 and futuro1Bid < precioMaxGanBidFuturo1 and (precioMaxGanBidFuturo1-futuro1Bid)>0.11 : #true
            ...
            limitTrueMax = float(self.clientR.fix.securitysList[self.botData["futuro2"]]["highLimitPrice"])
            limitTrueMin = float(self.clientR.fix.securitysList[self.botData["futuro2"]]["lowLimitPrice"])
            self.log.info(f"limitTrue: {limitTrue}, limitTrueMax:{limitTrueMax}, limitTrueMin:{limitTrueMin}")
            limitBidFuturo1 = precioMaxGanBidFuturo1-self.botData["varGan"]
    ...
    async def  verificar_futuro1_bid(self):
        ...
        if verificarOrdenCreada["status"]==True:
            ...
            if verificarFuturo1["status"]==True: 
                self.log.info("tengo orden creada y el bid1 es mio")
                ...
                if verificarFuturo2["status"]==True and verificarPase["status"]==True:
                    ...
                    self.botData["indices_futuros"][self.botData["futuro1"]] = {"BI": verificarFuturo1["indice"]}
                    ...
                    if calcularLimit>0:
                        ...
                        #modificar orden
                            ...
                            if self.botData["ordenOperada"] == False:
                                ...
                                modificarOrden = await self.clientR.modificar_orden(orden['orderId'], orden['clOrdId'],1, 2, 
                    ...
                    if self.botData["ordenOperada"] == False:
                        ...
                        borrarOrden = await self.clientR.cancelar_orden(orden['orderId'], orden['clOrdId'], 1, orden['leavesQty'], self.botData["futuro1"])
                ...
                if verificarFuturo2["status"]==True and verificarPase["status"]==True:
                    ...
                    if calcularLimit>0:
                        ...
                        if round(orden['price'],1) != round(limitAskFuturo1,1) or orden['leavesQty'] != sizeOrder:
                            self.log.info("es diferente entonces mando a actualizar")
                            modificarOrden = await self.clientR.modificar_orden(orden['orderId'], orden['clOrdId'],2, 2, self.botData["futuro1"], sizeOrder, limitAskFuturo1)
                            self.log.info(f"orden modificada {modificarOrden}")
                        else:
                            self.log.info("es lo mismo asi q no actuaklizo ")
                else:
                    ...
                    if calcularLimit>0:
                        ...
                        #modificar orden
                            if self.botData["ordenOperada"] == False:
                                self.log.info(f"orden operada = false, enviar a modificar orden")
                                self.log.info("es diferente entonces mando a actualizar")
                                modificarOrden = await self.clientR.modificar_orden(orden['orderId'], orden['clOrdId'],1, 2, self.botData["futuro2"],sizeOrder, limitBidFuturo2)
                                self.log.info(f"orden modificada {modificarOrden}")
                            else:
                                self.log.info("orden operada en proceso")
                else:
                ...
                if verificarFuturo2["status"]==True and verificarPase["status"]==True:
                    ...
                    if calcularLimit>0:
                        ...
                        if round(orden['price'],1) != round(limitAskFuturo2,1) or orden['leavesQty'] != sizeOrder :
                            ...
                            if self.botData["ordenOperada"] == False:
                                self.log.info(f"orden operada = false, enviar a modificar orden")
                                modificarOrden = await self.clientR.modificar_orden(orden['orderId'], orden['clOrdId'], 2, 2, self.botData["futuro2"], sizeOrder, limitAskFuturo2)
                                self.log.info(f"orden modificada {modificarOrden}")
                            else:
                                self.log.info("orden operada en proceso")
                        else:
    ...
    async def guardar_posiciones(self, posiciones):
        ...
        for posicion in posiciones:
            ...
            #{"BI": posicion["buySize"], "OF": posicion["sellSize"]}
                self.botData["posiciones"][self.botData["futuro1"]]["BI"] = posicion["buySize"]
                self.botData["posiciones"][self.botData["futuro1"]]["OF"] = posicion["sellSize"]
            if posicion["tradingSymbol"] == self.botData["futuro2"]:
                self.botData["posiciones"][self.botData["futuro2"]]["BI"] = posicion["buySize"]
                self.botData["posiciones"][self.botData["futuro2"]]["OF"] = posicion["sellSize"]
            
    async def  ejecutar_bot(self):
        self.log.info(f"ejecutando bot id: {self.botData['id_bot']} en cuenta: {self.botData['cuenta']}")
        ...
        arraySymbols = self.botData["symbols"]
        suscribir = await self.clientR.suscribir_mercado(arraySymbols)
        if suscribir["status"]==True:
            self.log.info("suscribir mercado ok")
            self.botData["botIniciado"] = True 
            self.log.info("bot iniciado ok")
            #en este punto ya tengo los tickers y los securitys
            while True: 
                time.sleep(0.1)
                #con esto detengo el bot en dado caso la variable pase a True
           ...
           #     self.log.info(f"voy a verificar si esta activo lo de soloEscucharMercado: {self.botData['soloEscucharMercado']} ")
                ...
                self.log.info("verificar ordenes operadas ")
                self.log.info(f"self.botData: {self.botData}")
                if self.botData["ordenOperada"]==False:
                    if self.botData["llegoTickers"] == True:
                        self.log.info("hubo un cambio de mercado entonces debo ejecutar los movimientos")
                        self.log.info("_tickers",self._tickers)
                        ordenes =  await self.verificar_ordenes()
                        self.botData["llegoTickers"] = False
                        ...
                        tiempoLimitStatus = finTimeStatus-self.botData["inicioTime"]
    ...
    def  run(self):
        ...
        loop.run_until_complete(self.ejecutar_bot())
        loop.close()
      
    def pasar_orden_operada(self, details, type=0):
        self.log.info("pasando orden operada")
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        loop.run_until_complete(self.verificar_orden_operada(details, type))
        loop.close()
