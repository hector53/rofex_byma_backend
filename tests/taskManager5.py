...
class TaskManager:
    ...
    async def get_bot_description_by_id(self, id):
        for task in self.tasks._queue:
            if task.id == id: 
                print("task descripcion: ", task.id)
                break
async def main():
    ...
    await task_manager.get_bot_description_by_id(3)
...
class Task:
    def __init__(self, id):
        ...
        self.id = id
        self.descripcion = "hola soy la descripcion del bot: "+str(self.id)
